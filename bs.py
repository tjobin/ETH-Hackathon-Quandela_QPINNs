...
class Trainer:
    ...
    def _create_optimizer(self) -> Optional[torch.optim.Optimizer]:
        """Create optimizer based on config. Returns None if model has no trainable parameters."""
        # Count trainable parameters
    ...
    def _create_scheduler(self) -> Optional[torch.optim.lr_scheduler._LRScheduler]:
        ...
        if self.config.training.lr_decay is None or self.optimizer is None:
            return None
    ...
    def train(self) -> np.ndarray:
        ...
        for epoch in range(1, self.config.training.epochs + 1):
            ...
            # Skip optimizer step if model has no trainable parameters
            ...
            # Backward pass (only if model is trainable)
            if self.optimizer is not None:
                loss.backward()
                self.optimizer.step()
            ...
            if epoch == 1 or epoch % self.config.log_every == 0:
                ...
                try:
                    ...
                    print(f"  Warning: Fourier evaluation failed: {e}")
